...
class ValueIteration:
    ...
    def step(self):
        ...
        self.v_fn = {tuple(s.tolist()): self.step_s(s) for s in self.env.states}
        self.n_steps += 1
        if self.verbose:
    ...
    def step_s(self, s, return_argmax=False):
        ...
        """
        self.env.s = s
        s_index = self.env.states_indices[tuple(s.tolist())]
        actions = self.env.actions[s_index]
        if isinstance(self.env, GridWorld):
            v = np.array(list(self.v_fn.values()))
            r = np.array([[self.env.step(action=a, new_state=x, perform_action=False) for x in self.env.states]
                          for a in actions])
            tmp = self.gamma * v + r if actions else np.array([])
            values = [tmp[i] @ self.env.transition_p[self.env.states_indices[tuple(s.tolist())]][tuple(a.tolist())]
    ...
    def get_v0(states):
        ...
        return {tuple(s.tolist()): v for v, s in zip(values, states)}
